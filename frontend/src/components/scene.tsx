...
export function Scene() {
  ...
  const { particles, originalPositionTexture, particlePositions } = useMemo(() => {
    ...
    const geometry = new THREE.TorusKnotGeometry(1.2, 0.3, 256, 24)
  ...
  useFrame((state, delta) => {
    ...
    if (pointsRef.current) {
      ...
      pointsRef.current.rotation.y += 0.06 * dt
      pointsRef.current.rotation.x += 0.03 * dt
    }
  })
  ...
  return (
    <>
      ...
      <EffectComposer>
        ...
        <N8AO quality="medium" aoRadius={0.5} intensity={1.5} color="black" />
      </EffectComposer>
    </>
  )
}
