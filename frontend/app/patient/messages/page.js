  ...
  useEffect(() => {
    ...
    async function loadCases() {
      const { data } = await supabase
        .from("cases").select("id,created_by,created_at")
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="w-72 shrink-0 rounded-xl overflow-hidden flex flex-col"
          ...
          <div className="px-4 py-3 shrink-0"
            ...
            <p className="text-xs text-slate-400 mt-0.5">{cases.length} case thread{cases.length !== 1 ? "s" : ""}</p>
          </div>
            ...
            {threads.length === 0 ? (
              ...
              return (
                <button key={thread.case_id} onClick={() => setCaseId(thread.case_id)}
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0"
                        style={{ background: active ? "#0f766e" : "linear-gradient(135deg,#94a3b8,#64748b)" }}>
                        C
                      </div>
                      <span className="text-xs font-semibold" style={{ color: active ? "#0f766e" : "#334155" }}>
                        Case {thread.case_id.slice(0, 8)}
                      </span>
                    </div>
                    {thread.unread > 0 && (
                  ...
                  {thread.last_body && (
                    <p className="text-xs text-slate-500 truncate pl-8">{thread.last_body}</p>
                  )}
                  <p className="text-xs text-slate-400 mt-0.5 pl-8">{fmtDate(thread.last_at)}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 rounded-xl overflow-hidden flex flex-col min-h-0"
          ...
          {caseId && (
            <div className="px-5 py-3 shrink-0 flex items-center gap-3"
              ...
              <div>
                <p className="text-sm font-semibold text-slate-700">Care Team</p>
                <div className="flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#10b981" }}/>
                  <p className="text-xs text-slate-400">Case {caseId.slice(0, 8)}</p>
                </div>
              </div>
            </div>
          )}
            ...
            {messages.length === 0 ? (
              ...
              return (
                <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                  ...
                  {mine && (
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white ml-2 mt-1 shrink-0"
          ...
          <div className="px-4 py-3 shrink-0 flex items-center gap-2"
            ...
            <div className="flex-1 flex items-center rounded-xl border gap-2 px-3 py-2 transition-all"
